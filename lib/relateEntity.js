const DIMENSIONS = ["people", "actions", "emotions", "topics"]
...
function normalize(row) {
  return {
    ...
    emotions: (row.entry_emotions ?? [])
      ...
      .filter((x) => x.id),
    topics: (row.entry_topics ?? [])
      .map((l) => ({ id: l.topic?.id, name: l.topic?.name }))
      .filter((x) => x.id),
  }
}
...
export function relateEntity(rows, dimension, id) {
  ...
  return {
    ...
    related: Object.fromEntries(others.map((d) => [d, tally(d)])),
